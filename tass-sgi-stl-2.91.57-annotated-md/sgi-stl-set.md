...
private:
  /* 注意，identity 定義於 <stl_function.h>，參見第7章，其定義為：
  template <class T>
...
public:
...
#else
  set(const value_type* first, const value_type* last) : t(Compare()) {
    t.insert_unique(first, last);
  }
  set(const value_type* first, const value_type* last, const Compare& comp) : t(comp) {
    t.insert_unique(first, last);
  }
  set(const_iterator first, const_iterator last) : t(Compare()) {
    t.insert_unique(first, last);
  }
  set(const_iterator first, const_iterator last, const Compare& comp) : t(comp) {
    t.insert_unique(first, last);
  }
  ...
  // 以下所有的 set操作行為，RB-tree 都已提供，所以 set 只要轉呼叫即可。
  // accessors:
  key_compare key_comp() const {
    return t.key_comp();
  }
  // 以下注意，set 的value_comp() 事實上為RB-tree 的key_comp()。
