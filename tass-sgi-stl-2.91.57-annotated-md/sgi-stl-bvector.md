...
struct __bit_const_iterator : public random_access_iterator<bool, ptrdiff_t> {
...
// 以下數行可能令你困惑。我們所要做的其實是宣告一個vector<T, Alloc>
// 的特化版本（partial specialization）— 如果我們擁有必要的編譯器支援的話。
// 否則我們就定義一個 class bit_vector，並令它使用default allocator。
...
protected:
  ...
  void initialize_range(InputIterator first, InputIterator last, input_iterator_tag) {
    start = iterator();
    finish = iterator();
    end_of_storage = 0;
  ...
  void initialize_range(ForwardIterator first, ForwardIterator last, forward_iterator_tag) {
    size_type n = 0;
  ...
  template <class InputIterator>
  void insert_range(iterator pos, InputIterator first, InputIterator last, input_iterator_tag) {
    for (; first != last; ++first) {
  ...
  template <class ForwardIterator>
  void insert_range(iterator position, ForwardIterator first, ForwardIterator last,
